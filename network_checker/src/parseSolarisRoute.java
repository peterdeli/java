...
public class parseSolarisRoute extends Thread {
    
    // options
    //
    //  stat [reset] [traffic] [tunnel] [route] [repeat]
    
    public parseSolarisRoute(){
        lines = new Vector();
    ...
    public parseSolarisRoute(String opt){
        option = opt;
        lines = new Vector();
        lineBuf = new StringBuffer();
        ...
        System.out.println("End of parseSolarisRoute (1)" );
    }
    ...
    protected void getConfig(){
        ...
        try {
            ...
            while ( true ) {
                ...
                if ( line == null ) {
                    break;
                } else if ( ! line.matches("^[0-9]+\\..*$") && ! line.matches( "^default.*") ){
                    continue;
                }
                ...
                switch ( splitLine.length ) {
                    case 6: 
                        ...
                        break;
                    case 5:
                         if ( Pattern.matches( SolarisHeadingFields.SolarisUpGateway,
                        splitLine[SolarisHeadingFields.SolarisGatewayCol] ) ){
                            gatewayInfo.put( SolarisHeadingFields.GatewayStateKey, SolarisHeadingFields.UP );
                            // IP Number
                        }
                        if ( Pattern.matches( SolarisHeadingFields.SolarisDefaultGateway,
                        splitLine[SolarisHeadingFields.SolarisDefaultGatewayCol] ) ){
                            gatewayInfo.put( SolarisHeadingFields.SolarisDefaultGateway, 
                                              splitLine[SolarisHeadingFields.SolarisDefaultGatewayIPCol] );
                            IP = splitLine[1];
                        }
                        break;
                }
            ...
            this.gateways.put( IP,  gatewayInfo );
            in.close();
            out.close();
    ...
    public static void main( String[] args ){
        ...
        parseSolarisRoute route = new parseSolarisRoute(args[0]);
        System.out.println( "Using Route command: " + route.getCommand());
    ...
    public void run() {
        ...
        System.out.println( "End of parseSolarisRoute run()" );
    }
